...
if (isset($_SESSION["uid"])) {
    ...
    $total_product_count = (int)$_POST['total_product_count'];
    ...
    '$address', '$city', '$state', '$zip', $total_product_count, $prod_total)";
        ...
        while($i<=$total_count){
           ...
           $sub_total=(int)$prod_price*(int)$prod_qty;
            ...
            if(mysqli_query($con,$sql1)){
                $del_sql="DELETE from cart where user_id=$user_id AND p_id=$prod_id";
                if(mysqli_query($con,$del_sql)){
                    var_dump($prod_id, $prod_qty);
                    $update_sql = "UPDATE `products` SET `Stock` = `Stock` - $prod_qty WHERE product_id=$prod_id;";
                    var_dump($update_sql);
